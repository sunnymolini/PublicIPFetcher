    ...
    class Program
    {
        ...
        public static string Output0(string IP)
        {
            ...
            if (System.IO.File.Exists(DataDir + @"\email.config"))
            {
                ...
                {
                    if      (m.Groups["key"].Value.ToUpper() == "server".ToUpper())     mailServer      = m.Groups["value"].Value;
                    else if (m.Groups["key"].Value.ToUpper() == "serverPort".ToUpper()) mailServerPort  = m.Groups["value"].Value;
                    else if (m.Groups["key"].Value.ToUpper() == "username".ToUpper())   mailUserName    = m.Groups["value"].Value;
                    else if (m.Groups["key"].Value.ToUpper() == "password".ToUpper())   mailPassword    = m.Groups["value"].Value;
                    else if (m.Groups["key"].Value.ToUpper() == "FromName".ToUpper())   mailFromName    = m.Groups["value"].Value;
                    else if (m.Groups["key"].Value.ToUpper() == "ToAddress".ToUpper())  mailTo          = m.Groups["value"].Value;
                }
